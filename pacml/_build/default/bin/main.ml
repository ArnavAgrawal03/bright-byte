...
(**[join_list] is a helper function that concates the cells in the csv to the set of the row in order to print the board*)
let rec join_list = function
  | [] -> ""
  | [ s ] -> s
  | s :: tl -> s ^ " " ^ join_list tl

(** Change the terminal settings to allow parsing of a single character. *)
let move_command () =
...
(**Helper funtion that prints the board and parses through the user's command*)
let play_game g :unit = 
  ...
  (**Starts the game using the play_game helper function *)
